...
def compute_calibration(true_labels, pred_labels, confidences, num_bins=10):
    ...
    for b in range(num_bins):
        ...
        if len(selected) > 0:
            bin_accuracies[b] = np.mean(np.array(true_labels)[selected]== np.array(pred_labels)[selected])
            bin_confidences[b] = np.mean(np.array(confidences)[selected])
...
def reliability_diagram(true_labels, pred_labels, confidences, num_bins=15,
...
def reliability_diagrams(results, num_bins=10,
    ...
    fig, ax = plt.subplots(nrows=nrows, ncols=ncols, sharex=True, sharey=True, 
                           figsize=figsize, dpi=dpi, constrained_layout=True,squeeze = False)
